...
class Detector:
    ...
    def start(self, modelResult: Array):
        ...
        while True:
            data = conn.recv(80)
            frame = np.asarray(pickle.loads(data))
            r = self.model.detect([frame], verbose=0)[0]
            ...
            maxIdx = tf.math.argmax(r["scores"])
            x = (r["rois"][maxIdx][1] + r["rois"][maxIdx][3]) / 2 - frame.shape[0] / 2
            y = (r["rois"][maxIdx][0] + r["rois"][maxIdx][2]) / 2 - frame.shape[1] / 2

            modelResult[0] = len(r["scores"]) != 0
            modelResult[1] = x * self.FACTOR_X
            modelResult[2] = y * self.FACTOR_Y
            modelResult[3] = id
            id = (id + 1) % 100000
